...
import { MAX_CALLS_PER_TOOL, QUERY_SIMILARITY_THRESHOLD } from '../config.js';
...
const DEFAULT_LIMIT_CONFIG: ToolLimitConfig = {
  ...
  similarityThreshold: QUERY_SIMILARITY_THRESHOLD,
};
...
export class Scratchpad {
  ...
  // In-memory cache of entries — avoids re-reading JSONL on every access
  private cachedEntries: ScratchpadEntry[] = [];
  ...
  canCallTool(toolName: string, query?: string): { allowed: boolean; warning?: string } {
    ...
    const latestOutcome = this.getLatestToolOutcomes().get(toolName);

    if (latestOutcome === 'auth/config failure') {
      return {
        allowed: false,
        warning: `Tool '${toolName}' previously failed due to auth/config issues in this query. ` +
          `Do not retry it now. Use a different tool family or tell the user the provider is not configured.`,
      };
    }

    if (latestOutcome === 'rate limited') {
      return {
        allowed: false,
        warning: `Tool '${toolName}' was rate limited earlier in this query. ` +
          `Do not immediately retry it. Use a different tool or proceed with available sources.`,
      };
    }
    ...
    if (query) {
      ...
      if (similarQuery) {
        ...
        return {
          allowed: latestOutcome !== 'no useful data returned',
  ...
  getToolUsageStatus(): ToolUsageStatus[] {
    ...
    const latestOutcomes = this.getLatestToolOutcomes();
    ...
    for (const [toolName, callCount] of this.toolCallCounts) {
      ...
      statuses.push({
        ...
        latestOutcome: latestOutcomes.get(toolName),
      });
    }
  ...
  formatToolUsageForPrompt(): string | null {
    ...
    const lines = statuses.map(s => {
      ...
      return `- ${s.toolName}: ${status}${s.latestOutcome ? ` · latest outcome: ${s.latestOutcome}` : ''}`;
    });

    return `## Tool Usage This Query\n\n${lines.join('\n')}\n\n` +
      `Note: If a tool isn't returning useful results after several attempts, or its latest outcome shows auth/config failure, rate limiting, or no data, do not keep retrying it immediately. Try a different tool/approach or clearly note the limitation to the user.`;
  }

  private getLatestToolOutcomes(): Map<string, string> {
    const outcomes = new Map<string, string>();
    for (const entry of this.readEntries()) {
      if (entry.type !== 'tool_result' || !entry.toolName) {
        continue;
      }
      outcomes.set(entry.toolName, this.classifyToolOutcome(entry.result));
    }
    return outcomes;
  }

  private classifyToolOutcome(result: unknown): string {
    const text = this.stringifyResult(result).toLowerCase();
    if (
      ...
      return 'auth/config failure';
    }
    if (text.includes('rate limited') || text.includes('too many requests') || text.includes('429')) {
      return 'rate limited';
    }
    if (
      text.includes('all providers failed') ||
      text.includes('called 0 data sources') ||
      text.includes('"resultcount":0') ||
      text.includes('"results":[]')
    ) {
      return 'no useful data returned';
    }
  ...
  /**
   * Get the set of cleared tool result indices (for retrieval indexing).
   */
  getClearedIndices(): Set<number> {
    return new Set(this.clearedToolIndices);
  }

  /**
   * Get all entries (for retrieval indexing of cleared results).
   */
  ...
  /**
   * Append-only write. Persists to disk and adds to in-memory cache.
   */
  ...
  /**
   * Read all entries from the in-memory cache.
   * Falls back to disk read only if cache is empty (e.g., after external corruption).
   */
  private readEntries(): ScratchpadEntry[] {
    ...
    // Fallback: read from disk if cache is empty (should only happen on init race)
    if (!existsSync(this.filepath)) {
      return [];
    }

    this.cachedEntries = readFileSync(this.filepath, 'utf-8')
      .split('\n')
      .filter((line: string) => line.trim())
      .map((line: string) => this.parseLine(line))
      .filter((entry: ScratchpadEntry | null): entry is ScratchpadEntry => entry !== null);
