...
export function checkApiKeyExists(apiKeyName: string): boolean {
  ...
  if (value && isConfiguredApiKeyValue(apiKeyName, value)) {
    return true;
  }
    ...
    for (const line of lines) {
      ...
      if (trimmed && !trimmed.startsWith('#') && trimmed.includes('=')) {
        ...
        if (key.trim() === apiKeyName) {
          ...
          if (isConfiguredApiKeyValue(apiKeyName, val)) {
            return true;
          }
        }
      }
...
function isConfiguredApiKeyValue(apiKeyName: string, value: string): boolean {
  const trimmed = value.trim();
  if (!trimmed || trimmed.startsWith('your-')) return false;
  if (/error:\s*\d{3}/i.test(trimmed) || /press enter to retry/i.test(trimmed)) return false;
  if (apiKeyName === 'HALAL_TERMINAL_API_KEY') return /^ht_[a-zA-Z0-9_-]+$/.test(trimmed);
  return true;
}
