...
- **act** with kind="upload" - Upload a local file into a file input
...
async function resolveUploadPath(filePath: string): Promise<string> {
  ...
  const roots = workspace
    ? [
        { cwd: workspace.rootDir, root: workspace.rootDir },
  ...
  throw lastError instanceof Error
    ? lastError
    : new Error(`Upload file must be inside the active workspace or current project: ${filePath}`);
}
...
const actRequestSchema = z.object({
  kind: z.enum(['click', 'type', 'press', 'hover', 'scroll', 'wait', 'upload']).describe('The type of interaction'),
  ...
  filePath: z.string().optional().describe('Path to a local file for upload actions'),
});
      ...
      switch (action) {
        ...
        case 'act': {
          ...
          switch (kind) {
            ...
            case 'upload': {
              if (!ref) {
                return formatToolResult({ error: 'ref is required for upload' });
              }
              if (!request.filePath) {
                return formatToolResult({ error: 'filePath is required for upload' });
              }
              const locator = resolveRefToLocator(p, ref);
              const uploadPath = await resolveUploadPath(request.filePath);
              await locator.setInputFiles(uploadPath, { timeout: 8000 });
              await p.waitForLoadState('networkidle', { timeout: 5000 }).catch(() => {});
              return formatToolResult({
                ok: true,
                uploaded: request.filePath,
                resolvedPath: uploadPath,
                hint: 'Upload successful. Call snapshot to inspect the updated page.',
              });
            }
