...
let _settingsFile: string | null = null;
function getSettingsFile(): string {
  if (!_settingsFile) _settingsFile = yassirPath('settings.json');
...
export function loadConfig(): Config {
  ...
  try {
    const content = readFileSync(getSettingsFile(), 'utf-8');
...
export function saveConfig(config: Config): boolean {
  try {
    const dir = dirname(getSettingsFile());
    if (!existsSync(dir)) {
      mkdirSync(dir, { recursive: true });
    }
    writeFileSync(getSettingsFile(), JSON.stringify(config, null, 2));
    return true;
  } catch {
    return false;
  }
