 ...
 *   bun run src/evals/run.ts --scorer              # Use multi-dimensional scorer (accuracy, completeness, source quality, conciseness)
 *   bun run src/evals/run.ts --sample 5 --scorer   # Sample + multi-dimensional scoring
 */
...
import { createScorer, computeAggregateStats, formatAggregateReport, type EvalScore } from './scorer.js';
...
function createEvaluationRunner(sampleSize?: number, useMultiDimScorer?: boolean) {
  return async function* runEvaluation(): AsyncGenerator<EvalProgressEvent, void, unknown> {
    // Initialize multi-dimensional scorer if requested
    const scorer = useMultiDimScorer ? createScorer() : null;
    ...
    for (const example of examples) {
      ...
      // Run multi-dimensional scorer if enabled
      let multiDimScore: EvalScore | undefined;
      if (scorer) {
        multiDimScore = await scorer.score({
          question: example.inputs.question,
          answer: outputs.answer,
          referenceAnswer: example.outputs.answer,
        });
        allScores.push(multiDimScore);
      }
      ...
      await client.createRun({
        ...
        extra: {
          ...
          evaluation: {
            ...
            ...(multiDimScore ? {
              accuracy: multiDimScore.accuracy.score,
              completeness: multiDimScore.completeness.score,
              sourceQuality: multiDimScore.sourceQuality.score,
              conciseness: multiDimScore.conciseness.score,
              overallScore: multiDimScore.overall.score,
              verdict: multiDimScore.overall.verdict,
            } : {}),
          },
        },
      });
      ...
      const comment = multiDimScore
        ? `${evalResult.comment || ''} [${multiDimScore.overall.verdict}: acc=${multiDimScore.accuracy.score.toFixed(1)} comp=${multiDimScore.completeness.score.toFixed(1)} src=${multiDimScore.sourceQuality.score.toFixed(1)}]`
        : (evalResult.comment || '');
      yield {
        type: 'question_end',
        question,
        score: multiDimScore ? multiDimScore.overall.score : (typeof evalResult.score === 'number' ? evalResult.score : 0),
        comment,
      };
    }

    // Print aggregate report if multi-dimensional scoring was used
    if (allScores.length > 0) {
      console.log('\n' + formatAggregateReport(computeAggregateStats(allScores)));
    }
...
async function main() {
  ...
  const useMultiDimScorer = args.includes('--scorer');

  // Create the evaluation runner with the sample size
  const runEvaluation = createEvaluationRunner(sampleSize, useMultiDimScorer);
