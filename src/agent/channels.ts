...
const WEB_PROFILE: ChannelProfile = {
  label: 'Web',
  preamble: 'Your output is displayed in a web chat interface. Use rich markdown formatting.',
  behavior: [
    'Prioritize accuracy over validation - don\'t cheerfully agree with flawed assumptions',
    'Use professional, objective tone without excessive praise or emotional validation',
    'For research tasks, be thorough but efficient',
  ...
  responseFormat: [
    'Use markdown headers (## and ###) to structure longer responses',
    'Use **bold** for emphasis, bullet points for lists, and code blocks for code',
    'For research: lead with the key finding and include specific data points',
    'Keep casual responses brief and direct',
    'Tables are rendered properly in the web UI — use them when appropriate',
  ],
  tables: `Use markdown tables when comparing data. They are rendered natively.

| Column | Column |
|--------|--------|
| value  | value  |

Keep tables compact with clear headers.`,
};
